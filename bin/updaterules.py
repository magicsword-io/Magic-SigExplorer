...
def main():
    rules = [
        ...
        },
        {
            ...
            'is_tar': True,
            'extracted_file_path': 'rule_files/community-rules/community.rules',
            'final_file_path': 'rule_files/snort3-community.rules'
        },
        {
            'url': 'https://rules.emergingthreats.net/open/suricata-5.0/emerging-all.rules',
            'file_path': 'rule_files/suricata-emerging-all.rules'
        }
    ]
                ...
                if not file_exists or old_hash != new_hash:
                    ...
                    if rule.get('is_tar', False):
                        try:
                            ...
                            extracted_file_path = rule.get('extracted_file_path')
                            final_file_path = rule.get('final_file_path')
                            if extracted_file_path and final_file_path:
                                extracted_file_exists = os.path.exists(extracted_file_path)
                                extracted_file_hash = get_file_hash(extracted_file_path) if extracted_file_exists else None
                                final_file_exists = os.path.exists(final_file_path)
                                final_file_hash = get_file_hash(final_file_path) if final_file_exists else None

                                if not final_file_exists or final_file_hash != extracted_file_hash:
                                    shutil.move(extracted_file_path, final_file_path)
                                    print(f"Renamed {extracted_file_path} to {final_file_path}.")
                                else:
                                    print(f"No update needed for {final_file_path}.")
